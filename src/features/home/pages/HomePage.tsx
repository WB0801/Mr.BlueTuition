...
const entries = [
  ...
  { label: '学费', description: '缴费与待开收据', path: '/fees', icon: '费', showsReceiptCount: true },
        ...
        {entries.map((entry) => (
          <Link className="home-entry" to={entry.path} key={entry.path}>
            ...
            <span className="entry-content">
              ...
              {'showsReceiptCount' in entry && (
                <span className="receipt-reminder">待开收据 {receiptCount.data ?? '—'}</span>
              )}
            </span>
          </Link>
        ))}
